...
test.describe( '@custom-post-type-widget-blocks latest-posts', () => {
	...
	test( 'insert block via Toggle block inserter / list select', async ( { editor, page } ) => {
		...
		await expect(
			page.locator( '.block-editor-inserter__panel-content' ).getByLabel( 'Custom Post Type Widget Blocks', { exact: true } ).getByRole( 'option', { name: 'Latest Posts (Custom Post Type)' } )
		).toBeVisible();

		await page.locator( '.block-editor-inserter__panel-content' ).getByLabel( 'Custom Post Type Widget Blocks', { exact: true } ).getByRole( 'option', { name: 'Latest Posts (Custom Post Type)' } ).click();
		await page.waitForTimeout(1000);
		await expect(
			page.locator( 'ul[data-type="custom-post-type-widget-blocks/latest-posts"]' )
		).toBeVisible();

		const content = await editor.getEditedPostContent();
		expect( content ).toBe(
			`<!-- wp:custom-post-type-widget-blocks/latest-posts /-->`
		);
